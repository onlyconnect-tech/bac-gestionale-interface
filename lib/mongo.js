...
export default class Mongo {
  ...
  async doInsertLocationIfNo(db, location){
    ...
    const resUpdate = await db.collection('localita').updateOne(location, { $set: location}, { upsert: true });
    ...
    // find becuse yet
    const resFind = await db.collection('localita').findOne(location);
  ...
  async doInsertAnagrafica(db, anagrafica) {
    ...
    const resUpdate = await db.collection('anagrafica').updateOne({ sequenceNumber: anagrafica.sequenceNumber }, { $set: anagrafica}, { upsert: true });
    
    if(resUpdate.upsertedCount === 1) {
        // ok inserted
        return { op: 'INSERT', anagraficaId: resUpdate.upsertedId._id };
    }
    ...
    // find becuse yet
    const resFind = await db.collection('anagrafica').findOne({ sequenceNumber: anagrafica.sequenceNumber });
   ...
   */
    ...
    doInsertAnagraficaOLD(db, anagrafica) {
